...
\label{mdf_k_eqn}
\end{equation}
where $\gamma_k = \max(0, \varepsilon/k)$. Now if \Eq{mdf_k_eqn} is solved with implicit method, the solution must keeps its positivity. Similar technique can be easily generalized to other reaction equations.
...
\begin{enumerate}
\item Runge-Kutta methods are a family of iterative methods used in temporal discretization for the approximation solutions of ODEs. They can be either implicit and explicit. The general Runge-Kutta methods take the form 
\begin{eqnarray}
y_{n+1} = y_n + h\sum_{i=1}^sb_ik_i\\
k_i = f(t_n+c_ih, y_n + h\sum_{j=1}^{s}a_{ij}k_j)
\end{eqnarray}
A specific Runge-Kutta method can be determined by putting all the coefficients in the Butcher tableau\cite{}:
\begin{equation}
\begin{array}{c|cccc}
...
\end{array}
\end{equation}
Note that this method does not have a step control parameters. It is also not suitable for the solution of stiff equations due to its bounded region of absolute stability. It has been proved that explicit Runge-Kutta methods can never be A-stable.
...
\end{equation}
\end{enumerate}
It has been proved that explicit methods can never be A-stable. An implicit multistep method can only be A-stable if their order is at most $2$, while an A-stable Runge-Kutta can have arbitrarily high order \cite{Hairer1993}. The difficulty of exceeding second order of an A-stable multistep methods is called Dahlquist barrier \cite{Dahlquist1963, Dahlquist1956}.
